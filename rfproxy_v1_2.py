...
from ryu.app.rflib.ipc.IPC import *
from ryu.app.rflib.ipc.MongoIPC import *
from ryu.app.rflib.ipc.RFProtocol import *
from ryu.app.rflib.openflow.rfofmsg_v1_2 import *
from ryu.app.rflib.ipc.RFProtocolFactory import RFProtocolFactory
...
ipc = MongoIPCMessageService(MONGO_ADDRESS, MONGO_DB_NAME, RFPROXY_ID)
table = Table()
datapaths = Datapaths()
...
def flow_delete(dp_id, address, netmask, src_hwaddress):
	...
	conf_flow(datapaths.get(dp_id), ip=address, mask=netmask,
				src_hw=src_hwaddress, instruction=TMP)
	log.info("ofp_flow_mod(tmp) was sent to datapath (dp_id=%s), (addr=%s)", dpid_to_str(dp_id), address)
	...
	def on_packet_in(self, ev):
		...
		for f in msg.match.fields:
			if f.header == ofproto_v1_2.OXM_OF_IN_PORT:
				in_port = f.value
			if f.header == ofproto_v1_2.OXM_OF_ETH_TYPE:
				_eth_type = f.value
		...
		if is_rfvs_int(dpid):
			...
			if dp_port is not None:
				...
				log.debug("Unmapped RFVS port (vs_id=%s, vs_port=%d)", dpid_to_str(dpid), in_port)
		# If the packet came from a switch, redirect it to the right RFVS port
			...
			if vs_port is not None:
				...
				log.debug("Unmapped datapath port (dp_id=%s, dp_port=%d)", dpid_to_str(dpid), in_port)
